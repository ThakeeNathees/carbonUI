...
SYMBOLS['$uiXt_p$'        ] = ['unsigned int*',     'var&/*int*/',        lambda param : f'(unsigned int*){param}.operator int64_t*()'    ] ## FIXME:
SYMBOLS['$iXt_p$'         ] = ['int*',              'var&/*int*/',        lambda param : f'(int*){param}.operator int64_t*()'             ] ## FIXME:
SYMBOLS['$const_flXXt_p$' ] = ['const float*',      'var&/*float*/',      lambda param : f'(const float*){param}.operator double*()'      ] ## FIXME:
SYMBOLS['$flXXt_p$'       ] = ['float*',            'var&/*float*/',      lambda param : f'(float*){param}.operator double*()'            ] ## FIXME:
SYMBOLS['$flXXt_r$'       ] = ['float&',            'var&/*float*/',      lambda param : f'*(float*)&({param}.operator double&())'        ] ## FIXME:
...
SYMBOLS['$const_vec2_r$'  ] = ['const ImVec2&',    'var&/*Vec2*/',        lambda param : f'*{param}.cast_to<Vec2>()'                      ]
SYMBOLS['$const_vec2_p$'  ] = ['const ImVec2*',    'var&/*Vec2*/',        lambda param : f'&{param}.cast_to<Vec2>()->operator ImVec2&()'  ]
SYMBOLS['$vec2_r$'        ] = ['ImVec2&',          'var&/*Vec2*/',        lambda param : f'*{param}.cast_to<Vec2>()'                      ]
SYMBOLS['$vec2$'          ] = ['ImVec2',           'var/*Vec2*/',         lambda param : f'*{param}.cast_to<Vec2>()',          lambda ret : f'newptr<Vec2>({ret})'  ]

SYMBOLS['$const_vec4_r$'  ] = ['const ImVec4&',    'var&/*Vec4*/',        lambda param : f'*{param}.cast_to<Vec4>()',          lambda ret : f'newptr<Vec4>({ret})'  ]
SYMBOLS['$const_vec4_p$'  ] = ['const ImVec4*',    'var&/*Vec4*/',        lambda param : f'&*{param}.cast_to<Vec4>()->operator ImVec4&()'     ]
SYMBOLS['$vec4&$'         ] = ['ImVec4&',          'var&/*Vec4*/',        lambda param : f'*{param}.cast_to<Vec4>()',          lambda ret : f'newptr<Vec4>({ret})'  ]
SYMBOLS['$vec4$'          ] = ['ImVec4',           'var/*Vec4*/',         lambda param : f'*{param}.cast_to<Vec4>()',          lambda ret : f'newptr<Vec4>({ret})'  ]
